...
fct = dirname + 'silicon_crt.condtens'
RYDBERG = 0.5
doping_level = 0.01
tau = 1.0e-14
...
for iT, T in enumerate(Tr):
    (N[iT], L0[iT], L1[iT], L2[iT], Lm11[iT]) = bandlib.fermiintegrals(epsilon, dos, vvdos, mur=np.array([mur[iT]]), Tr=np.array([T]), dosweight=data.dosweight, cdos=cdos)
...
sigma *= tau * 1e-5 # kS / cm
seebeck *= 1e6  # uV / K
kappa *= tau * 1e-2 # W / cm / K
