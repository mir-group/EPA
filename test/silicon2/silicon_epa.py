...
epsilon, dos, vvdos, cdos = BL.BTPDOS(eband, vvband, erange=[data.fermi - ecut, data.fermi + ecut], npts=round(2 * ecut / deltae), scattering_model='uniform_tau')
...
for iT, T in enumerate(Tr):
    mur[iT] = BL.solve_for_mu(epsilon, dos, _nelect - doping_level, T, data.dosweight, refine=True)
...
for iT, T in enumerate(Tr):
    (N[iT], L0[iT], L1[iT], L2[iT], Lm11[iT]) = BL.fermiintegrals(epsilon, dos, vvdos, mur=np.array([mur[iT]]), Tr=np.array([T]), dosweight=data.dosweight, cdos=cdos, scattering_model = 'epa', scattering_file = fepa)
...
for iT, T in enumerate(Tr):
    (sigma[iT], seebeck[iT], kappa[iT], Hall[iT]) = BL.calc_Onsager_coefficients(np.array([[L0[iT]]]), np.array([[L1[iT]]]), np.array([[L2[iT]]]), np.array([mur[iT]]), np.array([T]), volume, np.array([[Lm11[iT]]]))
