...
!-----------------------------------------------------------------------
!
!      Converter from EPI method to EPA approximation
!
!      Reads parameters from standard input and electron-phonon coupling
!          matrix elements from file
!      Averages electron-phonon coupling matrix elements over k and q
!          for each pair of bands or in energy bins within VBM and CVM
!          energy windows or generates distributions of electron-phonon
!          coupling matrix elements in VBM and CVM energy windows
!      Writes averaged electron-phonon coupling matrix elements or
!          distributions of electron-phonon coupling matrix elements to
!          file
!
!      Written by Georgy Samsonidze on 2015-01-28 -- 2015-05-11
!
!      Compilation: add the following to qe-6.1/PHonon/PH/Makefile
!
!      epi2epa.x : epi2epa.o libph.a $(PWOBJS) $(LRMODS) $(LIBOBJS)
!              $(LD) $(LDFLAGS) -o $@ epi2epa.o libph.a \
!                       $(LRMODS) $(PWOBJS) $(QEMODS) $(LIBOBJS) $(LIBS)
!              - ( cd ../../bin ; ln -fs ../PHonon/PH/epi2epa.x . )
!
!-----------------------------------------------------------------------
...
program epi2epa
  ...
  if (ijob .eq. 2) then
    write(6, 201) ('v', ll = 1, 5), ('c', ll = 1, 5)
201 format(<nwin>(5x, "e", a, " (eV)", 4x, "e", a, "' (eV)", &
        3x, "<|g", a, "|^2> (eV^2)", 2x, "count", a, 5x, "weight", a))
    do jj = 1, nemin
      do kk = 1, nemin
        write(6, 202) ((jj - 0.5d0) * de(ii), (kk - 0.5d0) * de(ii), &
            sum(gavg(:, kk, jj, ii)), gnum(kk, jj, ii), gtot(kk, jj, ii), &
            ii = 1, nwin)
202     format(<nwin>(2f12.6, e18.8, i8, f12.6))
      enddo
    enddo
  endif
  ...
  elseif (ijob .eq. 2) then
    ...
    enddo
    write(uno, 203) (wavg(nu), nu = 1, nmodes)
203 format(<nmodes>e18.8)
    do ii = 1, nwin
      do jj = 1, ne(ii)
        do kk = 1, ne(ii)
          write(uno, 204) ii, jj, kk, (gavg(nu, kk, jj, ii), &
              nu = 1, nmodes)
204 format(3i8, <nmodes>e18.8)
        enddo
      enddo
    enddo
  elseif (ijob .eq. 3) then
